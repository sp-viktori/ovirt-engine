...
public class TransferDiskImageCommand<T extends TransferDiskImageParameters> extends BaseImagesCommand<T> implements QuotaStorageDependent {
    ...
    protected String prepareImage(Guid vdsId) {
        ...
        // For managed block storage with NBD, skip Image.prepare: VDSM looks up the storage
        // domain in its connected-domains list, and MBS domains are not listed there, so
        // PrepareImage fails with "Storage domain does not exist". StartNbdServer will provide
        // the path and does not require a prior prepare for MBS.
        if (usingNbdServer()) {
