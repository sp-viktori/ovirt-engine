...
public class TransferDiskImageCommand<T extends TransferDiskImageParameters> extends BaseImagesCommand<T> implements QuotaStorageDependent {
    ...
    private void cleanupOrphanMbsConversionVolume(ManagedBlockStorage managedBlockStorage, Guid sdId, VDS vds,
        ...
        if (progress == MbsConversionStartProgress.CONNECTED_ON_HOST
            ...
            mbsConversionTryManagedBlockDisconnect(managedBlockStorage, volId);
        }
        mbsConversionTryManagedBlockDeleteVolume(managedBlockStorage, volId);
    }
    ...
    private void mbsConversionTryManagedBlockDisconnect(ManagedBlockStorage managedBlockStorage, Guid volId) {
        ...
        try {
            ...
            log.warn("Orphan cleanup: managed block DISCONNECT_VOLUME for {} failed: {}", volId, e);
        }
    }

    private void mbsConversionTryManagedBlockDeleteVolume(ManagedBlockStorage managedBlockStorage, Guid volId) {
        List<String> extraParams = Collections.singletonList(volId.toString());
        try {
            ...
            log.warn("Orphan cleanup: managed block DELETE_VOLUME for {} failed: {}", volId, e);
        }
    }
